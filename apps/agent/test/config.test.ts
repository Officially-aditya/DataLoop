...
test("places the default artifact manifest inside an overridden library directory", () => {
  const config = getAgentConfig({
    AGENT_ARTIFACT_LIBRARY_DIR: "/tmp/dataloop-agent-knowledge"
  } as NodeJS.ProcessEnv);

  assert.equal(config.artifactLibraryDir, "/tmp/dataloop-agent-knowledge");
  assert.equal(config.artifactManifestPath, "/tmp/dataloop-agent-knowledge/manifest.json");
});
