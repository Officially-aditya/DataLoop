...
async function main() {
  ...
  const metadataURI = process.env.BOOTSTRAP_ARTIFACT_METADATA_URI ?? "0g://artifact-metadata/excel.json";
  const metadataHash = process.env.BOOTSTRAP_ARTIFACT_METADATA_HASH ?? ethers.id("dataloop:artifact-metadata:excel:v1");
  const storageURI = process.env.BOOTSTRAP_ARTIFACT_STORAGE_URI ?? "0g://artifact-marketplace/excel.md";
